var passport = require('passport-strategy')
    ...
    , extractors = require('./extract_jwt');
...
function JwtStrategy(options, verify) {
    ...
    if (this._secretOrKeyProvider === 'ENV_JWT_SECRET') {
    ...
    if(options.jwtFromRequest === 'versionOneCompatibility'){
        this._jwtFromRequest = extractors.versionOneCompatibility({});
    }
        ...
        if (secretOrKeyError) {
            ...
            JwtStrategy.JwtVerifier(token, secretOrKey, self._verifOpts, function(jwt_err, payload) {
                if (jwt_err) {
                    ...
                    if(self.options && self.options.idField) {
                        if(!payload.id) {
                            payload.id = payload[self.options.idField];
                        }
    
                        if(!payload.id) {
                            return self.fail('no id specified for the user');
                        }
                    }

                    try {
                        if (self._passReqToCallback) {
                            self._verify(req, null, null, payload, verified);
                        } else {
                            self._verify(null, null, payload, verified);
                        }
                    } catch(ex) {
                        self.error(ex);
                    }
